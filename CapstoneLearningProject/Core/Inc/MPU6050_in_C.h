...
void MPU6050Init()
{
	  ...
	  SetSlaveAddr(MPU6050_DEFAULT_ADDRESS);
	  I2C_StartWrite(2);
	  I2C_TransmitByte(MPU6050_RA_PWR_MGMT_1);
	  I2C_TransmitByte(0x00);
	  I2C_Stop();

	// Prepare register power mgmt 1 register,  Clock source & Disable sleep mode
	uint8_t mpu6050_pwr_mgmt_1_config = (0 << MPU6050_PWR1_SLEEP_BIT) | MPU6050_CLOCK_PLL_XGYRO;
	// 1. Specify the addr of the device to read from
	SetSlaveAddr(MPU6050_DEFAULT_ADDRESS);
	// 2. Declare # of bytes to write and transmite the register / value pairs
	I2C_StartWrite(6);
	  // 2.1 Set Accelerometer to 4G config
	I2C_TransmitByte(MPU6050_RA_ACCEL_CONFIG);
	I2C_TransmitByte(MPU6050_ACCEL_FS_4 << 3);
	  // 2.2 Set gyro scale to 250
	I2C_TransmitByte(MPU6050_RA_GYRO_CONFIG);
	I2C_TransmitByte(MPU6050_GYRO_FS_1000 << 3);
	  // 2.3 Set clk src & disable sleep
	I2C_TransmitByte(MPU6050_RA_PWR_MGMT_1);
	I2C_TransmitByte(mpu6050_pwr_mgmt_1_config);
...
//    	Temperature |= 0xF000;
//    }
	// Calculation taken from datasheet, pg 30
	float Temperature_F = (((float)Temperature) / 340.0) - 159.82;
	Temperature_F = (Temperature_F * (9.0/5.0)) + 32.0;

	return Temperature_F;
}

signed int MPU6050_ReadRAWTemperature(void)
{
	signed int Temperature = 0;
	// Example of Multiple Byte Read
	// 1. Specify the addr of the device to read from
